...
import { Form, Input, Select, DatePicker, Button, Card, Typography, Layout, Row, Col, Divider, message } from 'antd';
...
const DeanWorkloadNew: React.FC = () => {
  ...
  const onFinish = async (values: WorkloadForm) => {
    try {
      ...
      navigate('/dean/workload');
    } catch (error: any) {
      console.error('Error creating workload:', error);
      message.error(error.response?.data?.message || 'ไม่สามารถเพิ่มภาระงานได้');
    } finally {
      setLoading(false);
    }
  };
    ...
    <Layout style={{ minHeight: '100vh', background: theme.background }}>
      ...
      <Layout style={{ height: 'calc(100vh - 70px)' }}>
        ...
        <Layout style={{ padding: theme.spacing.xl, overflow: 'auto' }}>
          <Content style={{ maxWidth: '1200px', margin: '0 auto', padding: `0 ${theme.spacing.xl}` }}>
            <div style={{ 
              ...
              <Button
                ...
                onClick={() => navigate('/dean/workload')}
                style={{ 
                  padding: 0,
            ...
            <Card
              ...
              bodyStyle={{ padding: `${theme.spacing.xxl} ${theme.spacing.xl}` }}
            >
              <Form
                form={form}
                layout="vertical"
                ...
                <Row gutter={[32, 24]}>
                  <Col span={24}>
                    <Form.Item
                      name="title"
                      label="หัวข้อภาระงาน"
                      rules={[{ required: true, message: 'กรุณากรอกหัวข้อภาระงาน' }]}
                      style={{ width: '100%' }}
                    >
                      <Input 
...
export default DeanWorkloadNew; 