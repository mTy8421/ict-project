...
const ViceDeanStrategyWorkload: React.FC = () => {
  ...
  const columns = [
    {
      ...
      render: (text: string, record: Workload) => (
        <a onClick={() => navigate(`/vice-dean/strategy/workload/${record.id}`)}>{text}</a>
      ),
    },
    {
      title: 'แผนก',
    ...
    },
    {
      ...
      render: (_: any, record: Workload) => (
        <Space size="middle">
          <Button type="link" onClick={() => navigate(`/vice-dean/strategy/workload/${record.id}/edit`)}>
            แก้ไข
          </Button>
  ...
  return (
    <Layout style={{ minHeight: '100vh', background: theme.background }}>
      <ViceDeanHeader role="รองคณบดีฝ่ายยุทธศาสตร์" />
      <Layout style={{ height: 'calc(100vh - 70px)' }}>
        <ViceDeanNavbar basePath="/vice-dean/strategy" />
          ...
          <Content style={{ 
            ...
            <div style={{ 
              ...
              <Button
                type="primary"
                icon={<PlusOutlined />}
                onClick={() => navigate('/vice-dean/strategy/workload/new')}
                style={{
...
export default ViceDeanStrategyWorkload; 